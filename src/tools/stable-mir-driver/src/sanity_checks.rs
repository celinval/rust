...
pub fn test_traits() -> TestResult {
    ...
    for trait_impl in stable_mir::all_trait_impls().iter().map(stable_mir::trait_impl) {
        check(
...
pub fn test_crates() -> TestResult {
    ...
    check_equal(stable_mir::find_crate(&local.name.as_str()), Some(local), "local crate mismatch")
}
...
fn check_body(body: stable_mir::mir::Body) {
    for bb in body.blocks {
        ...
        black_box(matches!(bb.terminator, stable_mir::mir::Terminator::Goto { .. }));
    }
